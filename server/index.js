...
const JWT_SECRET = 'super-secret-key-for-local-tracker'; // In real app, use env var
...
function logAction(userId, action, message) {
  db.run(`INSERT INTO audit_logs (user_id, action, details) VALUES (?, ?, ?)`, [userId, action, message], (err) => {
  ...
  try {
    ...
    db.run(`INSERT INTO users (username, password_hash) VALUES (?, ?)`, [username, hashedPassword], function(err) {
      ...
      defaults.forEach(cat => {
        db.run(`INSERT INTO categories (user_id, name, monthly_budget) VALUES (?, ?, ?)`, [newUserId, cat, 0]);
      });

      res.json({ message: "Registered successfully", id: newUserId });
    });
  } catch (error) {
...
app.post('/api/transfer', authenticateToken, (req, res) => {
  ...
  db.serialize(() => {
    ...
    db.get(`SELECT name FROM accounts WHERE id = ? AND user_id = ?`, [from_account_id, req.user.id], (err, fromAcc) => {
      if (!fromAcc) { db.run('ROLLBACK'); return res.status(400).json({ error: "Invalid source" }); }
      
      db.get(`SELECT name FROM accounts WHERE id = ? AND user_id = ?`, [to_account_id, req.user.id], (err, toAcc) => {
        if (!toAcc) { db.run('ROLLBACK'); return res.status(400).json({ error: "Invalid dest" }); }
        ...
        db.run(`UPDATE accounts SET balance = balance - ? WHERE id = ?`, [amount, from_account_id], (err) => {
          ...
          db.run(`UPDATE accounts SET balance = balance + ? WHERE id = ?`, [amount, to_account_id], (err) => {
            ...
            logAction(req.user.id, 'TRANSFER', msg);
...
app.post('/api/accounts', authenticateToken, (req, res) => {
  ...
  db.run(`INSERT INTO accounts (user_id, name, type, balance) VALUES (?, ?, ?, ?)`,
    [req.user.id, name, type, balance], function(err) {
    if (err) return res.status(500).json({ error: err.message });
    logAction(req.user.id, 'CREATE_ACCOUNT', `Created ${type} account '${name}' with initial balance ₱${balance}`);
...
app.delete('/api/accounts/:id', authenticateToken, (req, res) => {
  db.run(`DELETE FROM accounts WHERE id = ? AND user_id = ?`, [req.params.id, req.user.id], function(err) {
    if (err) return res.status(500).json({ error: err.message });
    logAction(req.user.id, 'DELETE_ACCOUNT', `Deleted account ID: ${req.params.id}`);
...
app.post('/api/incomes', authenticateToken, (req, res) => {
  ...
  db.serialize(() => {
    ...
    db.run(`INSERT INTO incomes (user_id, source_name, amount, month, year, account_id) VALUES (?, ?, ?, ?, ?, ?)`, 
      [req.user.id, source_name, amount, month, year, account_id], function(err) {
      if (err) { db.run(`ROLLBACK`); return res.status(500).json({ error: err.message }); }
      
      const newIncomeId = this.lastID;
      if (account_id) {
        db.run(`UPDATE accounts SET balance = balance + ? WHERE id = ? AND user_id = ?`, [amount, account_id, req.user.id], (err) => {
          if (err) { db.run(`ROLLBACK`); return res.status(500).json({ error: err.message }); }
          db.run(`COMMIT`);
          logAction(req.user.id, 'ADD_INCOME', `Added income '${source_name}' of ₱${amount} deposited to Account ID ${account_id}`);
          res.json({ id: newIncomeId, source_name, amount, month, year, account_id });
        });
      } else {
        db.run(`COMMIT`);
        logAction(req.user.id, 'ADD_INCOME', `Added income '${source_name}' of ₱${amount} (No Account)`);
...
app.delete('/api/incomes/:id', authenticateToken, (req, res) => {
  db.run(`DELETE FROM incomes WHERE id = ? AND user_id = ?`, [req.params.id, req.user.id], function(err) {
    if (err) return res.status(500).json({ error: err.message });
    logAction(req.user.id, 'DELETE_INCOME', `Deleted income ID: ${req.params.id}`);
    res.json({ deleted: true });
  });
});
...
app.post('/api/recurring', authenticateToken, (req, res) => {
  ...
  db.run(`INSERT INTO recurring_expenses (user_id, name, expected_amount, category, auto_pay, day_of_month) VALUES (?, ?, ?, ?, ?, ?)`,
    [req.user.id, name, expected_amount, category, auto, day], function(err) {
    logAction(req.user.id, 'ADD_PLANNED', `Added planned expense '${name}' for ₱${expected_amount}`);
...
app.delete('/api/recurring/:id', authenticateToken, (req, res) => {
  db.run(`DELETE FROM recurring_expenses WHERE id = ? AND user_id = ?`, [req.params.id, req.user.id], function(err) {
    logAction(req.user.id, 'DELETE_PLANNED', `Deleted planned expense ID: ${req.params.id}`);
    res.json({ deleted: true });
  });
});
...
app.post('/api/transactions', authenticateToken, (req, res) => {
  ...
  db.serialize(() => {
    ...
    db.run(`INSERT INTO transactions (user_id, amount, category, description, date, account_id) VALUES (?, ?, ?, ?, ?, ?)`,
      [req.user.id, amount, category, description, date, account_id], function(err) {
      if (err) { db.run(`ROLLBACK`); return res.status(500).json({ error: err.message }); }
      ...
      if (account_id) {
        db.run(`UPDATE accounts SET balance = balance - ? WHERE id = ? AND user_id = ?`, [amount, account_id, req.user.id], (err) => {
          if (err) { db.run(`ROLLBACK`); return res.status(500).json({ error: err.message }); }
          db.run(`COMMIT`);
          logAction(req.user.id, 'TRACK_EXPENSE', `Tracked ₱${amount} expense for '${description || category}' deducted from Account ID ${account_id}`);
          res.json({ id: newTxId, amount, category, description, date, account_id });
        });
      } else {
        db.run(`COMMIT`);
        logAction(req.user.id, 'TRACK_EXPENSE', `Tracked ₱${amount} expense for '${description || category}' (No Account)`);
...
app.delete('/api/transactions/:id', authenticateToken, (req, res) => {
  db.run(`DELETE FROM transactions WHERE id = ? AND user_id = ?`, [req.params.id, req.user.id], function(err) {
    if (err) return res.status(500).json({ error: err.message });
    logAction(req.user.id, 'DELETE_EXPENSE', `Deleted expense ID: ${req.params.id}`);
    res.json({ deleted: true });
  });
});
...
app.post('/api/categories', authenticateToken, (req, res) => {
  ...
  db.run(`INSERT INTO categories (user_id, name, monthly_budget) VALUES (?, ?, ?)`, [req.user.id, name, budget], function(err) {
    if (err) return res.status(500).json({ error: err.message });
    logAction(req.user.id, 'ADD_CATEGORY', `Added custom category '${name}' with budget ₱${budget}`);
...
app.delete('/api/categories/:id', authenticateToken, (req, res) => {
  db.run(`DELETE FROM categories WHERE id = ? AND user_id = ?`, [req.params.id, req.user.id], function(err) {
    if (err) return res.status(500).json({ error: err.message });
    logAction(req.user.id, 'DELETE_CATEGORY', `Deleted category ID: ${req.params.id}`);
    res.json({ deleted: true });
  });
});
...
app.put('/api/categories/:id', authenticateToken, (req, res) => {
  ...
  db.run(`UPDATE categories SET monthly_budget = ? WHERE id = ? AND user_id = ?`, [monthly_budget, req.params.id, req.user.id], function(err) {
    if (err) return res.status(500).json({ error: err.message });
...
app.post('/api/goals', authenticateToken, (req, res) => {
  ...
  db.run(`INSERT INTO goals (user_id, name, target_amount, deadline) VALUES (?, ?, ?, ?)`, [req.user.id, name, target_amount, deadline], function(err) {
    if (err) return res.status(500).json({ error: err.message });
    logAction(req.user.id, 'ADD_GOAL', `Added savings goal '${name}' for ₱${target_amount}`);
...
app.post('/api/goals/:id/contribute', authenticateToken, (req, res) => {
  ...
  db.run(`UPDATE goals SET current_amount = current_amount + ? WHERE id = ? AND user_id = ?`, [amount, req.params.id, req.user.id], function(err) {
    if (err) return res.status(500).json({ error: err.message });
    logAction(req.user.id, 'CONTRIBUTE_GOAL', `Contributed ₱${amount} to goal ID: ${req.params.id}`);
    res.json({ success: true });
  });
});

app.delete('/api/goals/:id', authenticateToken, (req, res) => {
  db.run(`DELETE FROM goals WHERE id = ? AND user_id = ?`, [req.params.id, req.user.id], function(err) {
    if (err) return res.status(500).json({ error: err.message });
    logAction(req.user.id, 'DELETE_GOAL', `Deleted goal ID: ${req.params.id}`);
    res.json({ deleted: true });
  });
});
